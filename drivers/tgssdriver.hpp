void drawPixel_tgss(unsigned short x, unsigned short y, unsigned short color)
{
    ...
    unsigned short lowWord = addressToWrite & 0xFFFF;
    ...
    //outb(highByte, 0x42A);
    //outb(highByte, 0x42A);
    asmOutW(lowWord, 0x428);
    asmOutb(highByte, 0x42A);
    TGSS_waitNotFull();         //wait for the video card write buffer to be not full before writing the pixel
    asmOutW(color, 0x42C);
}
...
unsigned int cursorPosToVramPos(unsigned int x, unsigned int y)
{
    //640x480 = 80 column, 60 rows
    unsigned int xpos = x * 8;
    return ((y*8)*480) + xpos;
}

//set the tgss address pointer to the input value
void tgss_setAddressPointer(unsigned int address)
{
    unsigned char highByte = (address >> 16) & 0xFF;
    unsigned short lowWord = address & 0xFFFF;
    outw(lowWord, 0x428);
    outb(highByte, 0x42A);
}