...
char waitChar()
{
    ...
    //graphicsModeDrawText_tgss(4*8,3*8,'z', 0x0FFF, 0x0000, true);
    while ((unsigned int)kdbData > 87 || kdbData == 0)
    {
        ...
        kdbData = asmInb(0x60);
        //kdbData = inb(0x60);
        //graphicsModeDrawText_tgss(4*8,3*8,'A', 0x0FFF, 0x0000, true);
...
string waitString(bool showProgress)
{
    ...
    //TODO: consider making at least backspace work. i dont have time to test and verify that so it will hve to wait
