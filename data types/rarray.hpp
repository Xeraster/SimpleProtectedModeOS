...
void rarray<T> :: clear()
{
   ...
   // }
    ...
    count = 0;
    maxSize = 0;
    m_init = false;     //if m_init is true and count = 0 and maxSize = 0, that *should* cause problems. Except adding this doesn't fix ANY bugs or issues at all whatsoever.
    free(array);
    //m_init = false;
    // shrink_to_fit();
    // maxSize = 10;
...
void rarray<T> :: resize(unsigned int newSize)
{
	...
	*(char*)0xB804D = 0x0E;
    ...
    {
        memcpy(array, oldPtr, oldSize);
    }
    else
    {
        memcpy(array, oldPtr, newSizeInBytes);
    }
    //*(char*)0xB8048 = 'R';
	//*(char*)0xB8049 = 0x0D;
