bool parseCommand(string input)
{
    ...
    else if (input.substr(0, 13) == "fpgavideotest")
    {
        ...
        if (input.length() > 0)
        {
            ...
            else if (entry == '7')
            {
                ...
                {
                    unsigned char status = inb(0x426);
                    unsigned char full = status & 0x30; //we're only concerned about bits 6, 5 and 4
                    int timeout = 0;
                    while (timeout < 1000 && (full != 0x00))
                    {
                        timeout++;
                        status = inb(0x426);
                        full = status & 0x30;
    ...
    else if (input.substr(0,12) == "fpgadatatest")
    {
        consoleNewLine();
        fpgaregisterfidelitytestCommand();
        return true;
    }

    else if (input.substr(0,14) == "fpgastringspam")
    {
        consoleNewLine();
...
bool colorTest(string input)
{
    ...
    {
        printString("v9958 TEXT2", 0x0F);
    }
    else if (videoType == 2)
    {
        printString("TGSS", 0x0F);
    }
    consoleNewLine(3);
    return true;
}
...
bool printCpuInfo(bool shortened)
{
    consoleNewLine(3);
...
//write then read a register over and over a bunch of times and see how many times it takes to get it wrong
bool fpgaregisterfidelitytestCommand()
{
    printString("What type of data test?", 0x0F);
    consoleNewLine();
    printString("1. R/W", 0x0F);
    consoleNewLine();
    printString("2. Generic read", 0x0F);
    consoleNewLine();
    printString("3. R/W but do it a lot of times", 0x0F);
    consoleNewLine();
    string optionStr = waitString(true);
    unsigned int option = stoi(optionStr);
    consoleNewLine();

    printString("delay between register operations: ", 0x0F);
    string delayStr = waitString(true);
    int delayNum = stoi(delayStr);
    consoleNewLine();

    printString("Number of iterations: ", 0x0F);
    string iterStr = waitString(true);
    int iterNum = stoi(iterStr);
    consoleNewLine();

    unsigned int errorCount = 0;
    unsigned char valToTry = 0x00;
    if (option == 1)
    {
        for (int i = 0; i < iterNum; i++)
        {
            for (int d = 0; d < delayNum; d++)
            {
                int wastetime = 4;
                wastetime = wastetime + 1;
            }

            // port 422 is a working R/W register that isnt connected to anything on the ice40 video card right now
            outb(valToTry, 0x422);

            // not exactly the most amazingly high-tech way to do a delay but it does work
            for (int d = 0; d < delayNum; d++)
            {
                int wastetime = 4;
                wastetime = wastetime + 1;
            ...
            unsigned char newVal = inb(0x422);
    ...
    {
        printString("What's the expected value? ", 0x0F);
        string theValStr = waitString(true);
        unsigned int theVal = stoi(theValStr, 0, 16);
        consoleNewLine();
    ...
    {
        printString("This option has no point. Just use option 1 with a higher iteration count. ", 0x0F);
        consoleNewLine();
    }

    printString("there were ", 0x0F);
    printInt(errorCount, 0x0F, false);
    printString(" errors in ", 0x0F);
    printInt(iterNum, 0x0F, false);
    printString(" iterations", 0x0F);
    return true;
}

//spam a bunch of words on the screen
bool fpgaStringSpamCommand()
{
    unsigned int x = 0;
    unsigned int y = 0;

    printString("String to print: ", 0x0F);
    string whatToPrint = waitString(true);
    consoleNewLine();

    printString("How many times to do it: ", 0x0F);
    string numTimesStr = waitString(true);
    int numTimes = stoi(numTimesStr);
    consoleNewLine();
    ...
    //{
        ...
        unsigned int xs = 0;
        // now that we have the information, draw the text. not tested hopefully this works
        for (int c = 0; c < (whatToPrint.length()*numTimes); c++)
        {
            // wait until it equals 40 i think that will provide the clearest text
            while (full != 0x40)
            {
            ...
            graphicsModeDrawText_tgss(x + (xs * 8), y + ys, whatToPrint.at(c%(whatToPrint.length()-1)), 0xFFFF, 0x0000, false);
            xs++;
            if (xs * 8 > 639)
            {
                xs = 0;
