...
/// @brief prints cpu info to the console
/// @param shortened set to true to make it only take up 1 line and leave out all the debugging details
/// @return 
bool printCpuInfo(bool shortened = false);

bool printVideoDriverInfo(int driverEnum, bool shortened = false)
{
    string vType;
    if (driverEnum == 0)
    {
    ...
    else if (driverEnum == 1)
    {
        vType = "v9958";
    }
    else if (driverEnum == 2)
    {
        vType = "TGSS";
    }
    else
    ...
    if (!shortened)
    {
        printString("Video driver: ", 0x0F);
    }
    printString(vType, 0x0F);
    return true;
}
