//convert ms-dos EGA color palette index into a 16 bit color value. ex: tgss_color_indexes[14h];
unsigned short tgss_color_indexes[] = { 
	0x0000, //0. black
	0x0015, //1. dark blue
	0x0540, //2. greenish yellow
	0x0555, //3. cyan
	0xA800, //4. red
	0xA815,	//5. magenta
	0xAA80,	//6. brown
...
void printChar(char msg, short color)
{
	...
	else if (videoType == 2)
	{
		if (msg > 31)
		{
			//graphicsModeDrawText()
			short letter = msg;
			graphicsModeDrawText_tgss(tgss_cur_x * 8, tgss_cur_y * 8, letter, tgss_color_indexes[color], 0x0000, true);
			tgss_cur_x++;
			if (tgss_cur_x >= 80)
			{
				tgss_cur_x = 0;
...
void clearScreen(int vgaMode)
{
	...
	{
		//set tgss vram pointer to 0
		asmOutW(0x0000, 0x428);
		asmOutb(0x00, 0x42A);
		for (int i = 0; i < 307200; i++)
		{
			//output the 16 bit R5G6B5 code for black and do it for every single pixel on the entire screen
			TGSS_waitNotFull();	//don't forget to do this
			asmOutW(0x0000, 0x42C);
		}
	}
	return;
}
...
void setCurPos(int posX, int posY)
{
	...
	else if (videoType == 2)
	{
		tgss_cur_x = posX;
		tgss_cur_y = posY;
		unsigned int address = cursorPosToVramPos(posX, posY);
		tgss_setAddressPointer(address);
	}
...
void insertChar(char msg, short color, int posX, int posY)
{
	...
	if (videoType == 2 || restOfLineIsBlank(posX, posY))	//one thing at a time. i dont want to do scrolling for tgss yet
	{
		printChar(msg, color);
	}
...
void insertChar(char msg, short color)
{
	...
	{
		insertChar(msg, color, tgss_cur_x, tgss_cur_y);
	}
...
void consoleNewLine(unsigned int videoMode)
{
	...
	{
		graphicsModeDrawText_tgss(4,2,'N', 0x0E, 0x00, true);
		tgss_cur_x = 0;
		tgss_cur_y++;
		unsigned int a = cursorPosToVramPos(tgss_cur_x, tgss_cur_y);
		tgss_setAddressPointer(a);
		graphicsModeDrawText_tgss(4,2,'N', 0x0D, 0x00, true);
	}
