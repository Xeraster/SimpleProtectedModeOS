...
- malloc finds an empty spot in the provided memory space to allocate a block of data of the provided size. It then updates the pointer array to indicate where in memory the end of the block is
...
- the initial array is going to be somewhere in low memory space due to a memory allocation system not being ready upon initial startup but after its first allocation resulting in a resize, it will end up in high memory space
...
- external programs could allocate memory in a non shitty way by using the kernel's malloc functions as long as they remember to clear their shit afterwards
...
note that correct implementation of everything on this page may actually enable a wider variety of GNU code to actually work such as math and string
*/
