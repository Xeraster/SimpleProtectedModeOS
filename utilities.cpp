...
void shiftCursor(int howFar)
{
	...
	{
		int pos = (tgss_cur_y * 80) + tgss_cur_x;
		...
		// OPTIONAL: clamp to max screen size if you have one
		// Example for 25 rows:
		// if (pos >= 80 * 25)
		//     pos = (80 * 25) - 1;
		...
		tgss_cur_y = pos / 80;
		tgss_cur_x = pos % 80;
		setCurPos(tgss_cur_x, tgss_cur_y);
	}
...
void backspaceScreen()
{
	...
	//actually its the same on all video drivers now
	/*if (videoType == 0)
	{
		printChar(' ', 0x0F);
	}
	...
	{
		graphicsModeDrawText_tgss(tgss_cur_x * 8, tgss_cur_y * 8, ' ', tgss_color_indexes[0x00], 0x0000, false);
		tgss_cur_x++;
		if (tgss_cur_x >= 80)
		{
			tgss_cur_x = 0;
